...
c. Plot the estimated probability as a function of the independent variable. Compare with the truth.
...
c. For the data below compute the MLE and construct confidence intervals.

d. Use bootstrap to construct the CI for the mean. Compare with c) and discuss.
```
```{r, message = FALSE, warning=FALSE}
...
b. Generate 20 samples from a Gamma distribution and plot a confidence ellipse of the inverse of Fisher information matrix around the ML estimates of the parameters. Also plot the theoretical values. Repeat the sampling several times. What do you observe?
