...
<!-- ``` -->
\BeginKnitrBlock{exercise}<div class="exercise"><span class="exercise" id="exr:unnamed-chunk-8"><strong>(\#exr:unnamed-chunk-8) </strong></span>Prove that the following subsets of $(0,1]$ are Borel-measurable by finding their measure.
...
a. Define a map (function) from $\Omega_1$ to
...
c. How would you in the same fashion define a map that would result
