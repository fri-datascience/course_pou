...
## Non-normal bivariate random variable
```{exercise, name = "Non-normal bivariate random variable", label = nnbivrv}
Find a non-normal bivariate random variable $(X, Y)$ with normal marginals.

Hint: there are many possible solutions. Try starting with $N(0, I)$, but set density to zero when $XY < 0$.
```

<div class="fold">
```{solution, echo = togs}
Let $\varphi(t) = \frac{1}{\sqrt{2\pi}} \exp(-t^2/2)$ be the univariate standard normal density.
The following bivariate density is one possible solution:
\begin{align}
    p_{XY}(x,y) = 2 \varphi(x)\varphi(y) \; \textrm{if}\; xy>0\textrm{, and 0 otherwise.}
\end{align}
We multiply by $2$ because we took away half the total mass from the original bivariate normal.
```
```{proof, echo = togs}
Let $f(x) = \int_{0}^\infty p_{XY}(x, y) dy$ and $g(x) = \int_{-\infty}^0 p_{XY}(x, y) dy$.
The marginal $p_X(x)$ is then:
\begin{align}
    p_X(x) = \int_{-\infty}^\infty p_{XY}(x, y) dy = \begin{cases}
        f(x), & x \ge 0, \\
        g(x),  & x < 0.
    \end{cases}
\end{align}
...
Simplify $f(x)$:
\begin{align}
    f(x) = \int_{0}^\infty p_{XY}(x, y) dy = 2\varphi(x) \int_{0}^\infty \varphi(y) dy = 2\varphi(x) \frac{1}{2} = \varphi(x)
\end{align}

\noindent
Simplify $g(x)$:
\begin{align}
    f(x) = \int_{-\infty}^0 p_{XY}(x, y) dy  2\varphi(x) \int_{-\infty}^0 \varphi(y) dy = 2\varphi(x) \frac{1}{2} = \varphi(x)
\end{align}

\noindent
Since $f(x) = g(x) = \varphi(x)$, we conclude that $p_X(x) = \varphi(x)$ regardless of $x$. Therefore $X \sim N(0, 1)$ and $Y \sim N(0, 1)$ by symmetry.
```
</div>
...
```{exercise}
The following is an adaptation of exercise 6.53 from [Ross: A first course in probability](https://www.pearson.com/en-us/pearsonplus/p/9780138027889).
Let $(X, Y)$ be a bivariate random variable with $X \sim \mathrm{Uniform}(0, 1)$ and $Y \sim \mathrm{Uniform}(0, 1)$.
Let $R = \sqrt{X^2 + Y^2}$ and $\Theta = \tan^{-1}(Y / X)$.
...
```{solution, echo = togs}
Define: $f([x, y]) = [r, \theta]$.
The inverse is $f^{-1}([r, \theta]) = [r \cos \theta, r \sin \theta]$. We want to find $p_{R\Theta}(r, \theta)$ by applying the change-of-variables formula:
\begin{align}
  p_{R\Theta}([r, \theta]) = p_{XY}(f^{-1}([r, \theta])) |\det J_{f^{-1}} ([r, \theta])|.
\end{align}
Since $X, Y \sim \mathrm{Uniform}(0, 1)$, we conclude that $p_XY([x, y]) = 1$ for all valid inputs $x, y \in [0, 1]$.

The Jacobian of $f^{-1}$ equals:
\begin{align}
...
\begin{align}
\det J_{f^{-1}} &= \cos \theta \cdot r \cos \theta - (\sin \theta \cdot (-\sin \theta)) \\
                &= r (\cos \theta)^2 + r (\sin \theta)^2 = r ((\cos \theta)^2 + (\sin \theta)^2) = r.
\end{align}

The final joint density is thus:
\begin{align}
  p_{R\Theta}([r, \theta]) &= p_{XY}(f^{-1}([r, \theta])) |\det J_{f^{-1}} ([r, \theta])| \\
                           &= 1 |r| = r.
\end{align}
